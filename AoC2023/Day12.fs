...
module Day12 =
    ...
    let rec isPartialAnswerValid (partialAnswers: int list) (numbers as (n::ns): int list): bool =
            ...
            | a::[] -> 
                if numbers.Length = 0 then 
                    ...
                    a <= n
            | a::pa -> if numbers.Length > 0 then (if a = n then isPartialAnswerValid pa ns else false) else false
    ...
    let prune ((line,numbers) : LineContainer) : (LineContainer * bool) option =
        ...
        // do this recursively?
        let allHashCountAndQuestionmark = line |> Array.filter (fun c -> c = '#' || c = '?') |> Array.length 
        let allHashCount = line |> Array.filter (fun c -> c = '#') |> Array.length 
        let numSum = numbers |> List.sum

        // needs to have periodsLength > 0!
        let hashesLength = line |> Array.skipWhile (fun c -> c = '.') |> Array.takeWhile (fun c -> c = '#') |> Array.length 
        let hashesOrQuestionmarksLength = line |> Array.skipWhile (fun c -> c = '.') |> Array.takeWhile (fun c -> c = '#' || c = '?') |> Array.length 
        let questionmarksLength = line |> Array.skipWhile (fun c -> c = '.') |> Array.takeWhile (fun c -> c = '?') |> Array.length 
        let periodsLength = line |> Array.takeWhile (fun c -> c = '.') |> Array.length
        let startStringLength = hashesLength + periodsLength
        let hasIndependentHashes = if startStringLength >= (line |> Array.length) then false else line[startStringLength] = '.'
        ...
        if allHashCount > numSum then
            ...
            if numSum > allHashCountAndQuestionmark then
                None
            else
                    ...
                    if periodsLength = 0 then
                        ...
                        | n::ns -> 
                            if allHashCountAndQuestionmark = 0 then
                                ...
                                if allHashCountAndQuestionmark < n then
                                    None
    ...
    let validateStringAdvanced (numbers: int list) (fixedString: char seq)  : bool =
        let firstPart = fixedString |> Seq.takeWhile (fun c -> c <> '?') |> String.Concat
        ...
        if partialAnswerArray = numbers then true 
        else isPartialAnswerValid partialAnswerArray numbers   
    ...
    let rec hej (fixedString: char list) : char list list =
        ...
        | a -> 
            if List.head a = '#' then
                (List.takeWhile ((=) '#') a) :: (hej ((List.skipWhile ((=) '#') a)))
            else
    ...
    let validateStringAdvanced2 (numbers: int list) (fixedString: char list)  : bool =
        ...
        let numberOfUnknowns = fixedString |> List.filter ((=) '?') |> List.length
        let numberOfHashes = fixedString |> List.filter ((=) '#') |> List.length
        let numSum = numbers |> List.sum
        let hashesLeftToUse = numSum - numberOfHashes


        if numberOfHashes > numSum then false
        elif hashesLeftToUse > numberOfUnknowns then false
        else
    ...
    let pruneStringsAndNumbers (strings: char array array) (numbers as (n::ns): int list) : (char array array) * (int list) =
        ...
        let hej = 
            ...
            |> Array.map (fun cs -> 
                ...
                let takens = 
                    cs
                    |> Array.take (cs.Length - restLength)

                let realTakens = if (Array.isEmpty takens |> not) && Array.last takens = '.' then takens else [||]
        ...
        if hej |> Array.forall (fun (a,_) -> matchesNumberExactly a n) then
    ...
    let rec permuteArrangement3 (inputStrings: LineContainer array) : int =
        if inputStrings |> Array.forall (fun (_,ns)-> List.isEmpty ns) then
            ...
            let beforePrune = 
                ...
                Array.map (fun (l,ns) ->
                    ...
                    | None -> ([|(l, ns)|])
                    | Some i ->
                        let combined = [|((Array.updateAt (i) '.' l), ns); ((Array.updateAt (i) '#' l), ns)|]
                        combined
    ...
    let validArrangements ((input, numbers): Input) : int =
        ...
        let numbers = numbers |> Array.toList
        let permuted = permuteArrangement2 numbers (input |> Seq.toArray |> Array.singleton) |> List.toArray
    ...
    let unfoldRecord ((string, numbers): Input) : Input =
        ...
        ([|newString ; [|'?'|]; newString ; [|'?'|]; newString ; [|'?'|] ; newString ;[|'?'|]; newString|] |> Array.concat |> String.Concat, numbers |> Array.replicate 5 |> Array.concat)
